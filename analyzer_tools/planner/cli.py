...
from analyzer_tools.planner.report import make_report, evaluate_alternate_model
...
def optimize(
    ...
    try:
        ...
        experiment = expt_from_model_file(
            model_file, simulator.q_values, simulator.dq_values
        )
        ...
        for param_val, info_gain, std_gain in results:
            click.echo(
                f"{param_val:>12.3f}     ΔH = {info_gain:>6.4f} ± {std_gain:>6.4f}"
            )
        ...
        make_report(
            json_file=os.path.join(output_dir, "optimization_results.json"),
...
def alternate_model(
    ...
    """
    Evaluate an alternate model against the simulated datasets from optimization.

    This command takes the results of a previous optimization and fits an alternate
    model to the simulated datasets to assess its performance.
    """
    ...
    try:
        click.echo("Evaluating alternate model...")
        click.echo(f"Model: {model_file}")
        click.echo(f"Result file: {result_file}")

        alternate_results_file = os.path.join(
            output_dir, "optimization_results_alternate.json"
        )
        evaluate_alternate_model(
            model_name=model_file,
            json_file=result_file,
            output_file=alternate_results_file,
            mcmc_steps=mcmc_steps,
            burn_steps=burn_steps,
        )

        click.echo(
            f"Alternate model evaluation completed. Results saved in {output_dir}"
        )

        make_report(
            json_file=alternate_results_file,
            output_dir=output_dir,
        )
...
def report(
    ...
    This command reads the optimization results from a JSON file and generates
    plots summarizing the information gain and simulated data.
    """
    setup_logging(verbose=verbose)
    try:
        make_report(
        ...
        click.echo(f"Report generated in directory: {output_dir}")
    except Exception as e:
...
main.add_command(alternate_model)
